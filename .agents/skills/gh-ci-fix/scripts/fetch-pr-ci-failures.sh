...
# If CI is in progress, polls every 60s (up to 30m) and returns as soon as
...
# Poll if in progress. Break as soon as at least one reproducible job has failed
# or the run completes.
if [ "$RUN_STATUS" = "in_progress" ] || [ "$RUN_STATUS" = "queued" ] || [ "$RUN_STATUS" = "waiting" ] || [ "$RUN_STATUS" = "pending" ]; then
  echo "CI is running on branch: $BRANCH - polling every 60s (timeout 30m)" >&2
  ...
  while true; do
    if [ "$(date +%s)" -gt "$DEADLINE" ]; then
      FAILED=$(gh api "/repos/$REPO/actions/runs/$RUN_ID/jobs?per_page=100" \
        --jq '[.jobs[] | select(.conclusion == "failure") | {databaseId: .id, name: .name}]')
      if [ "$(echo "$FAILED" | jq 'length')" -gt 0 ]; then
        break
      fi
      echo "STATUS: timeout"
      echo "CI still running after 30 minutes - no failures detected yet"
      exit 1
    fi

    RUN_NOW=$(gh api "/repos/$REPO/actions/runs/$RUN_ID" \
      --jq '{status: .status, conclusion: .conclusion}')
    JOBS_NOW=$(gh api "/repos/$REPO/actions/runs/$RUN_ID/jobs?per_page=100" \
      --jq '[.jobs[] | {databaseId: .id, name: .name, status: .status, conclusion: (.conclusion // "")}]')

    RUN_STATUS=$(echo "$RUN_NOW" | jq -r '.status')
    CONCLUSION=$(echo "$RUN_NOW" | jq -r '.conclusion')
    FAILED_NOW=$(echo "$JOBS_NOW" | jq '[.[] | select(.conclusion == "failure")]')
    ...
    if [ "$RUN_STATUS" = "completed" ]; then
      ...
      FAILED=$(echo "$FAILED_NOW" | jq '[.[] | {databaseId, name}]')
      break
    fi
